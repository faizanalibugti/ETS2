...
def proceesed_img(original_image):
            ...
            #cockpit view
            polygons = np.array([[200,395],[650,395],[650,430],[200,430]])
...
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
    while True:
      ...
      screen = cv2.resize(grab_screen(region=(536,98,1056,530)), (1056,530))
      ...
      for i,b in enumerate(boxes[0]):
        ...
        # Main detection 1 person (mop up) 3 car   6 bus  8 truck    7 train (tanker)  37 sports ball (tanker)   11 fire hydrant (weird trucks)   73 laptop (tankers)  61 cake (buses sometimes)
        if classes[0][i] == 1 or classes[0][i] == 3 or classes[0][i] == 6 or classes[0][i] == 8 or classes[0][i] == 7 or classes[0][i] == 37 or classes[0][i] == 11 or classes[0][i] == 73 or classes[0][i] == 61:
          cars = 1
          if scores[0][i] >= 0.35:
            ...
            if apx_distance <=0.7 and apx_distance >0.5:
              if mid_x > 0.4 and mid_x < 0.55:
                if x == True or y == True:
                  ReleaseKey(W)
                  PressKey(S)

            # If I see something closer than distance 0.5 within my AOI, friction brake then re-engage tacc
            elif apx_distance <=0.5 and apx_distance >=0.1:
              if mid_x > 0.4 and mid_x < 0.55:
                if x == True or y == True:
      ...
      if x == True:
        ...
        if lines is not None:
              ...
              try:
                    ...
                    # autosteer criteria for normal curves
                    f_l = [(l_x+(line_center*1.017))]
                    f_r = [(l_x+(line_center*0.03))]
                    #create a center point which is fixed technically 375, but we set higher to engineer out right bias
                    center_fixed =[390]
                    x_1 = int(l_x)
                    x_2 = int(r_x)
